...
export default class Calendar_h5 extends Taro.Component {
  constructor(props) {
    super(props);
  ...
  getPreNextArray(index, data) {
    const varChange = this.props.calendarType === "week" ? {
      PRE: PRE_WEEK,
      THIS: THIS_WEEK,
      NEXT: NEXT_WEEK
    } : {
    ...
    const dataChange = this.props.calendarType === "week" ? {
      PRE: "tempPreWeek",
      THIS: "tempThisWeek",
      NEXT: "tempNextWeek"
    } : {
  ...
  changeDate(e) {
    ...
    if (this.props.calendarType === "week") {
      ...
      if (showWeeks && showWeeks.length) {
        ...
        const newWeeks = this.getWeeks(oldWeeks[mapChangeWeekIndex[arrayIndex]][3].date);
      ...
      if (showMonths && showMonths.length) {
        ...
        const newMonths = this.getMonths(oldMonths[mapChangeMonthIndex[arrayIndex]][8].date);
  ...
  changeMonth(date) {
    if (this.props.calendarType === "week") {
      ...
      let newIndex = this.state.currentActive + 1 > 2 ? 0 : this.state.currentActive + 1;
  ...
  dealEveryDay({ i, currentDay, date }) {
    ...
    let dateData = {
      ...
      highlight: this.props.highlightWeekend ? [0, 6].includes(day.get("day")) : false
    };
    // tip
    ...
    // badge
    if (typeof this.props.badge === "string" && dateData.date === this.props.badge) {
    ...
    if (this.props.disabledDay instanceof Array) {
      this.props.disabledDay.forEach(item => {
  ...
  render() {
    const { thisWeek, thisMonth, preWeek, preMonth, nextWeek, nextMonth, showMonths, currentDate, currentActiveLines, currentYear } = this.state;
    const { highlightWeekend, highlightWeekendColor, activeColor, showType, backToToday, calendarType } = this.props;
    const weekGridComponent = ["周日", "周一", "周二", "周三", "周四", "周五", "周六"].map((week, index) => <View key={week}>
        <ClText align={"center"} size={"small"} lineSpacing={"small"} textColor={highlightWeekend ? [0, 6].includes(index) ? highlightWeekendColor : "black" : "black"}>
          {week}
        </ClText>
      </View>);
    const BGClassName = BG_COLOR_LIST[activeColor || "blue"];
    const oldTextClassName = TEXT_COLOR_LIST["gray"];
    const weeksComponent = preWeek.length ? [preWeek, thisWeek, nextWeek].map((week, index) => <SwiperItem key={week[3].date}>
            <ClGrid col={7}>
              {week.map(item => <View key={item.timeStamp} onClick={() => {
          this.clickDate({
            item
          });
        }}>
                  <ClLayout padding={"small"} paddingDirection={"vertical"}>
                    <ClFlex align={"center"} justify={"center"}>
                      <View style={{
                position: "relative"
              }} className={classNames(["day-normal", `${item.disabled ? BG_COLOR_LIST["gray"] : ""}`, `${item.special.show && !item.active ? BG_COLOR_LIST[item.special.color] : ""}`, {
                ...
                [oldTextClassName]: item.old
                  ...
                  ["cu-tag badge"]: item.badge.show
                        ...
                        <ClFlex align={"center"} justify={"center"} direction={"column"}>
                          <ClText size={12} align={"center"} cut={true} style={{ maxWidth: Taro.pxTransform(50) }} textColor={item.active && !item.disabled ? undefined : item.tipTopColor} text={item.tipTop} />
                          <ClText text={item.day} textColor={!item.active && !item.disabled && item.highlight ? highlightWeekendColor : undefined} />
                          <ClText size={12} align={"center"} cut={true} style={{ maxWidth: Taro.pxTransform(50) }} textColor={item.active && !item.disabled ? undefined : item.tipBottomColor} text={item.tipBottom} />
                        </ClFlex>
                      </View>
                    </ClFlex>
                  </ClLayout>
    ...
    const monthsComponent = [preMonth, thisMonth, nextMonth].every(item => item.length > 0) ? [preMonth, thisMonth, nextMonth].map((month, index) => <SwiperItem key={month[3].date}>
            <ClGrid col={7}>
              {month.map(item => <View key={item.timeStamp} onClick={() => {
          this.clickDate({
            item
          });
        }}>
    ...
    return <ClCard type={showType}>
        <View className={classNames([BG_COLOR_LIST["white"], "cuCalendar"])}>
          <ClLayout padding={"small"} paddingDirection={"bottom"}>
            <ClFlex justify={"between"} align={"center"}>
              <ClTip renderMessage={<View>
                    ...
                    <ClGrid col={4}>
                      {months.map((item, index) => <View key={item} className={classNames(["flex justify-center align-center"])}>
                          <ClLayout margin={"small"} marginDirection={"vertical"}>
                            <ClButton size={"small"} bgColor={"light-gray"} shadow={false} long text={item} onClick={() => {
                      this.selectDate(index);
                    }} />
                          </ClLayout>
                        </View>)}
                    </ClGrid>
                  </View>} mode={"click"} width={300} show={this.state.showMonths} onChange={() => {
              ...
              this.setState({
                ...
                <ClButton shape={"round"} shadow={false} size={"small"}>
                  <ClText className={"chooseDate"} lineSpacing={"normal"} text={currentDate} />
                </ClButton>
              </ClTip>
              {backToToday ? <View onClick={() => {
          ...
          <View className="week">
            ...
            <Swiper current={this.state.currentActive} skipHiddenItemLayout circular key={'key-' + Math.random()} style={{
            width: "100%",
            height: calendarType === "week" ? Taro.pxTransform(120) : Taro.pxTransform(120 * currentActiveLines)
          }} onChange={e => {
