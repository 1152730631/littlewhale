...
import "../index.scss";
export default class Calendar_weapp extends Taro.Component {
  render() {
    ...
    const [thisWeek, setThisWeek] = Taro.useState([]);
    const [preWeek, setPreWeek] = Taro.useState([]);
    const [nextWeek, setNextWeek] = Taro.useState([]);
    const [thisMonth, setThisMonth] = Taro.useState([]);
    const [preMonth, setPreMonth] = Taro.useState([]);
    const [nextMonth, setNextMonth] = Taro.useState([]);
    const [chooseDate, setChooseDate] = Taro.useState(today);
    const [currentDate, setCurrentDate] = Taro.useState(dealYearMonth(dayjs(today)));
    const [currentActive, setCurrentActive] = Taro.useState(1);
    const [currentActiveLines, setCurrentActiveLines] = Taro.useState(5);
    const [showMonths, setShowMonths] = Taro.useState(false);
    const [currentYear, setCurrentYear] = Taro.useState(dayjs().year());
    ...
    const weeksComponent = [preWeek, thisWeek, nextWeek].map((week, index) => <SwiperItem key={week[3].date}>
      <ClGrid col={7}>
        {week.map(item => <View key={item.timeStamp} onClick={clickDate.bind(this, {
          item,
    ...
    const monthsComponent = [preMonth, thisMonth, nextMonth].map((month, index) => <SwiperItem key={month[3].date}>
        <ClGrid col={7}>
          {month.map(item => <View key={item.timeStamp} onClick={clickDate.bind(this, {
          item,
          type: mapChangeWeekIndex[index]
        })}>
              <ClLayout padding={"small"} paddingDirection={"vertical"}>
    ...
    return <ClCard type={showType}>
      <View className={classNames([BG_COLOR_LIST["white"], "cuCalendar"])}>
        <ClLayout padding={"small"} paddingDirection={"bottom"}>
          <ClFlex justify={"between"} align={"center"}>
            <ClTip renderMessage={<View>
                ...
                </View>} mode={"click"} width={300} show={showMonths} onChange={() => {
        ...
        <View className="week">
          ...
          <Swiper current={currentActive} circular style={{
            width: "100%",
