...
export default class ClImagePicker extends Taro.Component {
  ...
  ChooseImage() {
    ...
    _chooseImage({
      ...
      success: res => {
        const selectArray = res.tempFilePaths.map(url => ({
          url,
          status: "none"
        }));
  ...
  render() {
    ...
    // const chooseImg = chooseImgObj || {};
    const maxPic = max || 0;
    const imgComponent = imgList.map((item, index) => <View className="padding-xs bg-img bg-gray" key={item.url} style={{ borderRadius: "6px", position: "relative" }} onClick={() => {
      this.viewImage.call(this, item.url);
    }}>
        <Image src={item.url} mode="widthFix" style={{
        width: "100%",
        position: "absolute",
        left: "50%",
        top: "50%",
        ...
        {item.status === "none" && !item.disabled ? <View className="cu-tag bg-red" onClick={e => {
        e.stopPropagation();
    ...
    return <View className={classNames("cu-form-group", className)} style={Object.assign({}, style)}>
        <View className="grid col-4 grid-square flex-sub">
          {imgComponent}
          {(maxPic === 0 || maxPic !== imgList.length) && <View className="padding-xs bg-gray" onClick={this.ChooseImage.bind(this)} style={{ borderRadius: "6px" }}>
