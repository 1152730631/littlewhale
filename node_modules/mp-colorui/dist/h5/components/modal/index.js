...
export default class ClModal extends Taro.Component {
  ...
  render() {
    ...
    const actionsComponent = actions.map((item, index) => <View className={`${index > 0 ? "solid-left" : ""} action margin-0 flex-sub`} key={"key-" + item.text} onClick={() => {
      onClick(index);
    }}>
    ...
    return <View className={classNames(`cu-modal ${this.state.showModal ? "show" : ""}`, this.props.className)} style={Object.assign({}, this.props.style)} onClick={() => {
