...
export default class ClTree extends Taro.Component {
  ...
  changeChecked(commonId = [], item) {
    ...
    const allLevelChildren = flatList.filter(child => {
      ...
      return levelIdArr.every((item, index) => item === levelArr[index]) && item.deepIndex < child.deepIndex;
    });
    ...
    if (checked) {
      ...
      while (i > 0) {
        ...
        result = result && current.children.every(child => child.checked && !child.someCheck);
    ...
    return <View style={{
      paddingLeft: Taro.pxTransform((item.deepIndex - 1) * 20)
        ...
        <ClFlex align={"center"}>
          ...
          {showCheck ? <ClCheckbox type={"normal"} shape={"round"} more={item.someCheck} color={color} checkboxGroup={[{
  ...
  render() {
    ...
    const listComponent = list.map(item => <View key={"key-" + item.commonId}>{this.renderItem(item)}</View>);
