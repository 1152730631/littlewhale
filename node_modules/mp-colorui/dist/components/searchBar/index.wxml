    ...
    <view style="{{anonymousState__temp4}}">
        <view class="{{anonymousState__temp5}}" style="{{anonymousState__temp6}}">
            <block wx:if="{{leftIcons}}">
                <view wx:key="$loopState__temp2" class="{{'cu-avatar round cuIcon-' + item.$original}}" bindtap="onIconClick" wx:for="{{loopArray65}}" wx:for-item="item" wx:for-index="index" data-e-tap-so="this" data-e-tap-a-a="{{index}}"></view>
            </block>
            <view class="{{'search-form ' + shape}}"><text class="cuIcon-search"></text>
                ...
                <block wx:if="{{clear && value}}">
                    <view catchtap="anonymousFunc0">
                        <cl-icon compid="{{$compid__834}}"></cl-icon>
                    </view>
                </block>
            </view>
            <block>
